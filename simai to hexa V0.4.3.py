...
def stage_action():
    ...
    if   re.match("^[1-6]to",temp):
        ...
        final_action_output.append(f"a s_move{change_object} {note_pos} {attribute} {axis} {round(cur_beat,4)} {duration} {amount} {easing}")
...
if overwrite_choice.lower() == "y" or overwrite_choice.lower() == "n" :
    ...
    if overwrite_choice.lower() == "y":
        ...
        for line in output_file.readlines():#find lines that are NOT start with a or n and put the line in storage[]
            ...
            if re.match("^a .+",line) or re.match("^n.+",line) or re.match("^-+",line) or re.match("^Generated by ",line) or re.match("^define StagePosition",line) or re.match("^a changestage.+",line): 
                log("Ignore: " + line)
                storage.append("")
            else:
                log("Append: " + line)
