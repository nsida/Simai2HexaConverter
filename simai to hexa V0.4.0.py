...
while cur < len(syllist):
    ...
    if re.match ("^E", syllist[cur]):#Detect "E"
        ...
        '''
    if re.findall("\((\d*\.)?\d+\)", syllist[cur]):#Detect "(bpm)"
        log("found BPM")
        ...
        if re.findall("\((\d*\.)?\d+\)",temp):
            ...
            log(f"BPM found is {now_bpm_temp}")
            temp_behind = temp.replace(now_bpm_temp,"")
            now_bpm = float(now_bpm_temp.replace("(","").replace(")",""))
            log(f"BPM have been changed to {now_bpm}")
            
    
        else:
            log("It is a BPM but could not recognise")
...
print("-"*50)
print("Generated by Simai2HexaConverter V0.4.0")
print("-"*50)
print("Warning:")
...
input("Press ANY key to close the converter")
