...
while cur < len(syllist):
    ...
    if re.match ("^E", syllist[cur]):
        ...
        print("End syllabus found, resetting beat to 0")
...
print ("-"*50)
overwrite_choice = input("Do you want to directly overwrite lines in chart.hard.txt?(Y/N)\nY: Lines that start with a or n in chart.hard.txt will be overwrited\nN: Newly generated lines will be added under the existing lines in chart.hard.txt")
if overwrite_choice == "Y" or overwrite_choice == "N" :
    ...
    if overwrite_choice == "Y":
        ...
        for line in output_file.readlines():#find lines that are NOT start with a or n and put the line in storage[]
            ...
            if re.match("^a s.+",line) or re.match("^n.+",line) or re.match("^-+",line) or re.match("^Generated by ",line) : 
                print("Delete: " + line)
                storage.append("")
            else:
                print("Preserve: " + line)
        ...
        output_file = open("chart.hard.txt","a",encoding="utf-8")  
    ...
    for line in final_note_output:#write all the lines saved in final_note_output[]
        output_file.writelines(line+"\n")
    output_file.writelines("-"*50+"\n")
    for line in final_action_output:#write all the lines saved in final_note_output[]
        output_file.writelines(line+"\n")    
    output_file.writelines("-"*50+"\n")  
    output_file.close()

else:
    print("Invalid response: Aborting changes")
